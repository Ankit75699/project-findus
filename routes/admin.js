...
var clist;
router.use("/manage_subcategory", (req, res, next) => {
  indexModel
    .fetchAll("category")
    .then((result) => {
      clist = result;
...
router.get("/manage_category", function (req, res, next) {
  res.render("manageCategory", { msg: "", sunm: req.session.sunm });
});
router.post("/manage_category", function (req, res, next) {
  var catnm = req.body.catnm;
  var caticon = req.files.caticon;
  var caticonnm = Date.now() + "-" + caticon.name;
  var caticonpath = path.join(
    __dirname,
...
router.get("/manage_subcategory", function (req, res, next) {
...
router.post("/manage_subcategory", function (req, res, next) {
  var catnm = req.body.catnm;
  var subcatnm = req.body.subcatnm;
  var subcaticon = req.files.subcaticon;
  var subcaticonnm = Date.now() + "-" + subcaticon.name;
  var subcaticonpath = path.join(
    __dirname,
    "../public/uploads/subcategoryicons",
    subcaticonnm
  );
  adminModel
    .managesubcategory(catnm, subcatnm, subcaticonnm)
    .then((result) => {
      if (result) {
        subcaticon.mv(subcaticonpath);
        res.render("managesubcategory", {
          msg: "Sub Cateory added successfully",
          clist: clist,
          sunm: req.session.sunm,
        });
      } else
        res.render("managesubcategory", {
          msg: "Sub Cateory already exists",
          clist: clist,
          sunm: req.session.sunm,
        });
    })
